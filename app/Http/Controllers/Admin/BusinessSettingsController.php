...
class BusinessSettingsController extends Controller
{
    ...
    public function landing_page_settings($tab)
    {
        ...
        {
            return view('admin-views.business-settings.landing-page-settings.heroslider');
        }
         else if($tab == 'ribboncms')
        {
            return view('admin-views.business-settings.landing-page-settings.ribboncms');
        }
    ...
    public function update_landing_page_settings(Request $request, $tab)
    {
        if($tab=='text')
        {
            DB::table('business_settings')->updateOrInsert(['key' => 'landing_page_text'], [
                'value' => json_encode([
                    ...
                    'ribbon_header'=>$request['ribbon_header'],
         ...
         else if($tab=='heroslider')
        {
            $data = [];
            $imageName = null;
            $heroslider = BusinessSetting::where('key', 'heroSlider')->first();
            if($heroslider)
            {
            ...
            array_push($data, [
                'img'=>$imageName,
                'title'=>$request->heroslider_title,
                'desc'=>$request->heroslider_desc
            ]);

            DB::table('business_settings')->updateOrInsert(['key' => 'heroSlider'], [
        ...
        else if($tab=='ribboncms')
        {
            $data = [];
            $imageName = null;
            $ribbonslider = BusinessSetting::where('key', 'ribboncms')->first();
            //echo $ribbonslider;die;
            if($ribbonslider->value)
            {
                $data = json_decode($ribbonslider->value, true);
            }
            if($request->has('image'))
            {
                $imageName = \Carbon\Carbon::now()->toDateString() . "-" . uniqid() . ".png";
            ...
            array_push($data, [
                'img'=>$imageName,
                'title'=>$request->ribbonslider_title,
                'desc'=>$request->ribbonslider_desc
            ]);

            DB::table('business_settings')->updateOrInsert(['key' => 'ribboncms'], [
                'value' => json_encode($data)
            ]);
            Toastr::success(trans('messages.landing_page_ribboncms_updated'));
        }
        else if($tab=='speciality')
        {
            $data = [];
