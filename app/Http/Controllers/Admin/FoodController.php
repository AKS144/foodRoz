...
class FoodController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        //Check if restaurant has comission or central comission
            $restaurant_id = $request->restaurant_id;
            $restaurant = Restaurant::find($restaurant_id);
            $comission=$restaurant->comission;
            if($comission)
            $comission=$comission;
            else
            $comission=BusinessSetting::where(['key' => 'admin_commission'])->first()->value;
        ...
        if (count($combinations[0]) > 0) {
            foreach ($combinations as $key => $combination) {
                ...
                $item['price'] = $item['price'] + ($comission / 100 * $item['price'] );

                array_push($variations_sell,$item);
            }
        }
        //combinations end
        $food->variations = json_encode($variations);
        $food->variations_sell = json_encode($variations_sell);
        $food->price = $request->price;
        $food->sell_price = $request->price + ($comission / 100 * $request->price );
    ...
    public function update(Request $request, $id)
    {
        ...
        $p->variations_sell = json_encode($variations_sell);
        $p->price = $request->price;
        $p->sell_price = $request->price + + ($comission / 100 * $request->price );
        ...
        $p->preparation_time = $request->preparation_time;
