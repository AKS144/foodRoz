...
class VendorController extends Controller
{
    ...
    public function updateRestaurantSettings(Restaurant $restaurant, Request $request)
    {
        ...
        if($request->comission_status)
        {
            ...
            //print_r($restaurant->foods);die;
            //$restaurant= new Restaurant();
            //$products = Food::where('restaurant_id',$restaurant->id)->all();
            //print_r($products);die;

            $products = $restaurant->foods;
            foreach ($products as $product) {
                $product->sell_price = $product->price + ($request->comission / 100 * $product->price );
                if($product->variations)
                {
                    ...
                    {
                        //print_r($variation['price']);
                        //echo "<br>";
                        $variation['price']= $variation['price'] + ($request->comission /100 * $variation['price']);
                        $variations[$key]['price'] = $variation['price'];
                        //print_r($variation['price']);
                        //echo "<br>";
                    ...
                    //print_r($variations);
                        //die();
                    $product->variations_sell=json_encode($variations,true);
                }
                $product->save();
            }
        }
