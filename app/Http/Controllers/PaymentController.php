...
class PaymentController extends Controller
{
    public function payment(Request $request)
    {
        ...
        //print_r($order->order_amount);die;

        $raz_order_id=$api->order->create(array('receipt' => $request->order_id, 'amount' => $order->order_amount * 100, 'currency' => 'INR', 'notes'=> $order->order_note));

        $raz_order_id=(string) $raz_order_id->id;

        session()->put('customer_id', $request['customer_id']);
        session()->put('order_id', $request->order_id);
        session()->put('raz_order_id', $raz_order_id);
