...
class Helpers
{
    ...
    public static function variation_price($product, $variation)
    {
        ...
        $result = 0;
        foreach (json_decode($product['variations_sell'], true) as $property => $value) {
    ...
    public static function product_data_formatting($data, $multi_data = false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                foreach (json_decode($item['variations_sell'], true) as $var) {
                    array_push($variations_sell, [
                        'type' => $var['type'],
                        'price' => (double)$var['price']
                    ]);
                }
                $item['variations_sell'] = $variations_sell;
            ...
            foreach (json_decode($data['variations_sell'], true) as $var) {
                array_push($variations_sell, [
                    'type' => $var['type'],
                    'price' => (double)$var['price']
                ]);
            ...
            $data['variations_sell'] = $variations_sell;
