                ...
                <table id="datatable"
                    ...
                    @foreach($orders as $key=>$order)
                        <tr class="status-{{$order['order_status']}} class-all">
                            ...
                            @php($base_price=($order['order_amount'] - $order['original_delivery_charge'] - $order['total_tax_amount'] + $order['restaurant_discount_amount'] + $order['coupon_discount_amount']))
                            
                            
                            @php($restaurant_price=round($base_price*100/(100+$comission),2) - $order['restaurant_discount_amount'])
                            <td>{{\App\CentralLogics\Helpers::format_currency($restaurant_price)}}</td>
